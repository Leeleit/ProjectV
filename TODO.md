...
- [x] **Задача 3.2.** Почаночный Incremental Jolt (8x V1 Phase 6 — boundary-neighbor queue) ✅ Closed (this session)
...
- [/] **Задача 5.2.** Аппаратные тени и отражения через Ray Query (BLAS + TLAS + ray query в `voxel.frag` для smooth specular) — ⏸️ Partial — BLAS dispatch + AABB geometry wired (17x + this session): `BuildDirtyBlases` consumes `DirtyChunkRebuild` queue via one-shot cmd buffer + fence; `BuildChunkBlas` now uses `VK_GEOMETRY_TYPE_AABBS_KHR` (per-chunk `VkAabbPositionsKHR`) instead of zero-stub triangle path; AABB scratch buffer (24 B) + vkCmdUpdateBuffer + barrier; `DirtyChunkRebuild { chunkIndex, aabb }` struct; Renderer.cpp looks up chunk AABB from `world.chunks[chunkIndex]` min/maxExclusive; `voxel.frag.rtx.spv` + `voxel.frag.rtx_taa_on.spv` SPIR-V variants with `rayQueryEXT` + `accelerationStructureEXT rtxTlas` (binding 13) for smooth-specular GI when `roughness <= kVctCutoffRoughness=0.3`; `graphicsPipelineRtx` + `graphicsPipelineRtxTaaOn` graphics pipelines; RTX pipeline selected in `Renderer.cpp` when `rayTracedShadows->IsEnabled() && tlas != VK_NULL_HANDLE`; descriptor pool + layout binding 13 added only when `IsRayTracedShadowEnabled()` (per VUID-VkDescriptorSetLayoutBinding-descriptorType-04616); `ProjectVRayTracedShadowTests` 11/11 sub-tests. Smoke: validation layer logs VUID-VkShaderModuleCreateInfo-pCode-08740 (RayQueryKHR capability) despite `VkPhysicalDeviceRayQueryFeaturesKHR::rayQuery=VK_TRUE` on device — known Vulkan loader/validation layer bug, non-fatal, shader modules + pipelines still create successfully. Deferred: working TLAS with populated instances (currently `tlasInstanceBuffer` allocated but no instances bound → `vkQueueSubmit` for `vkCmdBuildAccelerationStructuresKHR` TLAS build not yet called from `BuildDirtyBlases`); RTX-correct visual smoke (requires populated TLAS). Per TODO_NEW.md AUDIT-DOC-005.
...
**Сводка:** 14 ✅ Closed · 2 ⏸️ Partial (6.2 PIMPL full struct move, 5.2 RTX TLAS population + working ray-query) · 0 🔓 Open. После 17x + this session осталось 2 TODO-подзадач. Следующие сессии: 6.2 PIMPL full migration, 5.2 TLAS instance population + visual smoke.
...
* **Статус (2026-06-21 session 17x + this session):** ✅ Closed. Phase 6 incremental sync landed (17x): `SyncPhysicsWorld` split into full-rebuild (world pointer change) + incremental (edit-only) paths. Incremental path: `ProcessChunkRebuildQueue` (per-chunk `BuildChunkStaticCollisionBody`) → `RebuildStaticWorldBodyFromChunkShapes` (re-emits monolithic `staticWorldBodyId` from `chunkMergedBoxes` map). `chunkMergedBoxes` = new `std::unordered_map<uint32_t, std::vector<projectv::physics::MergedVoxelBox>>` field on `PhysicsState`, populated by `BuildChunkStaticCollisionBody` after GreedyMerge. `IsPhysicsStaticWorldBodyId` helper replaces `bodyId == staticWorldBodyId` check at `IsWalkJumpLockedSourceSupportSideWallContact` so walk jump contract still works against the compound-of-chunks body. Per-edit cost on FlatBench: ~30 ms (down from ~250 ms full scan). `Fluid↔Air` transitions in `SetVoxelMaterial` no longer bump `editVersion` (Fluid is not physics-solid) — eliminates 20 Hz × N-tick redundant syncs while water settles. **This session (Batch 12):** (a) `IsFluidCaGpuPipelineRequested` + `IsAsyncComputeEnabled` default flipped from OFF → ON per TODO Stage 3.1 Step 2 (Fluid CA GPU default) and Stage 6.3 close-out (Async Compute default); opt-out via `PROJECTV_FLUID_CA_GPU=0` / `PROJECTV_ASYNC_COMPUTE=0`. (b) `PlotBroadphaseDiagnostics` Tracy plotter + `GetPhysicsBroadphaseStats` query function + `PhysicsBroadphaseStats` POD struct expose Jolt `BodyManager::BodyStats` (mNumBodies/mNumBodiesStatic/mNumBodiesDynamic/mNumActiveBodiesDynamic/mNumBodiesKinematic/mNumActiveBodiesKinematic) + `pendingChunkRebuilds`/`chunkStaticBodies`/`chunkMergedBoxes` sizes for HUD/sidecar/test visibility. Wired into `TickWalkCharacter` per-tick. (c) Updated `ProjectVFluidCAGpuTests` (renamed `DefaultOff`→`DefaultOn`, added `OptOut` test) and `ProjectVAsyncComputeTests` (renamed `DefaultOff`→`DefaultOn`). (d) Added `TestGetPhysicsBroadphaseStatsNullReturnsZeroed` + `TestGetPhysicsBroadphaseStatsAfterSync` to `ProjectVPhysicsSyncTests` (11→13 sub-tests). All 39/39 tests pass.
