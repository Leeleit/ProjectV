...
% --- Титульный слайд (генерируется Beamer из \title/\author/\institute в header.tex) ---
\begin{document}
...
\begin{frame}[plain]
  ...
  \vspace{0.5em}
  \begin{flushright}
    \qrcode[height=1.5cm]{https://github.com/Leeleit/ProjectV}
  \end{flushright}
\end{frame}
...
\begin{frame}{Прототип: Тестовая лаборатория VoxelLab}
  ...
  \begin{center}
    \includegraphics[width=0.55\textwidth]{screenshots/voxel_lab.png}
  \end{center}
  \vspace{-0.3em}
  \scriptsize
  \begin{columns}[T]
...
% === Слайд 5: Анализ аналогов и ниша проекта ===
...
\begin{frame}{Анализ ограничений, рисков и безопасности}
  ...
  \begin{columns}[T]
    \begin{column}{0.48\textwidth}
      ...
      \begin{itemize}
        \item Гонка дескрипторов при частой смене сцен локализована через вызов
              \code{vkDeviceWaitIdle} в деструкторе. Полное исправление запланировано на Phase 5.
      \end{itemize}
    \end{column}
    \begin{column}{0.48\textwidth}
...
% === Слайд 12: Команда проекта и личный вклад ===
\begin{frame}{Распределение зон ответственности}
  ...
  \begin{table}
    ...
    \begin{tabular}{|l|l|l|p{0.42\textwidth}|}
      \hline
      \textbf{Участник} & \textbf{Роль на сцене} & \textbf{Компетенция} & \textbf{Зона ответственности в коде} \\
      \hline
      \textbf{Кадочников Л. (le1t)} & T2 Demo \& Стек & Архитектура, Q\&A host & Общая структура, PIMPL-AppState, мост ECS, snapshot-сохранения, hot shader reload \\
      \hline
      Тиммейт 1 & T1 Проблема, Цели & Сборка, тесты & CMake presets, развертывание CTest, скрипты автоматизации Runtime Smoke \\
      \hline
      Тиммейт 2 & T3 Архитектура, Код & Воксельный мир & Структуры чанков, материалы, Fluid CA, voxel raycast \\
      \hline
      Тиммейт 3 & (Q\&A сессия) & Рендеринг & CSM, PCF 5$\times$5, контактные тени, TAA \\
      \hline
      Тиммейт 4 & T6 Роадмап, Риски & Физика движка & Интеграция Jolt Physics, walk-контроллер игрока, режимы движения \\
      \hline
      Тиммейт 5 & T5 Фичи, Метрики & Ассеты, звук & glTF-пайплайн, Draco-декодер, интеграция miniaudio, сканирование плейлистов MP3 \\
      \hline
    \end{tabular}
  \end{table}
\end{frame}
