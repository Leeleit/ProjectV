...
> «Сейчас на экране вы видите нашу эталонную демо-сцену — Voxel Laboratory. Это пол в виде шахматной доски 18 на 18 вокселей с чередованием белого и светло-серого цветов. В центре стоит стеклянный шар радиуса 6 вокселей, внутри которого находится жидкость. Справа — непрозрачный якорь из тех же материалов, он нужен для стабильных теней от солнца.
>
> Сцена процедурно генерируется при старте за 200 миллисекунд. Это 27 чанков (3×3×3 grid по chunkSize=8), индексированных плоским массивом. Пол на Y=0, центр сферы в точке (0, 8, 0). Все размеры — в воксельных координатах, шаг сетки один метр.
>
...
|---|---|
| **VoxelLab / Voxel Laboratory** | Эталонная демо-сцена: пол 18×18, стеклянный шар **r=6**, жидкость, 27 чанков. |
...
- «Вот эта сцена. 27 чанков (3×3×3 grid), 200 мс генерация. Стеклянный шар r=6, жидкость внутри, пол-шахматка.»
...
> «Это наша reference-сцена. Пол — шахматная доска 18×18 вокселей, чередующиеся белый и серый. В центре — стеклянный шар радиуса 6 вокселей (per `VoxelLabShellConfig.radius=6`), внутри жидкость (70% заполнение). Справа — непрозрачный якорь для стабильных shadow casters. Генерируется процедурно за 200 мс. Используется для всех эталонных измерений: ctest, smoke 6/6 captures, FPS measurement (110-130 FPS, 7-9 мс debug).»
...
> «Сейчас в `music/` два трека: `Le1t - Palm Trees.mp3` и `Le1t - aCID.mp3`. Можно добавлять новые — плейлист пересканируется через 5 секунд. Поддерживается ТОЛЬКО MP3 (case-insensitive extension check); WAV/FLAC тихо игнорируются.»
...
│  • 27 чанков (3×3×3 grid), генерация 200 мс                           │
...
│  • Playlist scan: каждые 5 секунд, MP3 only (WAV/FLAC ignored)      │
