...
>
> Кратко об аналогах. Minecraft — коммерческий Java плюс OpenGL, только в 2026 году переходит на Vulkan. Minetest — open-source, но C++ плюс Lua и устаревший OpenGL. VoxelCore на чистом OpenGL. Veloren на Rust, но это action-RPG с упором на мультиплеер. Наш пробел: ни один open-source voxel не сочетает DOD, Vulkan 1.4 и C++26 в одном воспроизводимом фундаменте.
>
> **[Переход на 6 слайд — Архитектура]**
>
> Архитектурно: SDL3 → AppState → рендерер на Vulkan, физика на Jolt, ECS Flecs. Главная идея — данные-ориентированный дизайн: чанки 8 на 8 на 8 вокселей, плоский массив материалов в один байт на воксель, что даёт cache-friendly доступ. Жадный мешинг считается compute-шейдером: 6 проходов объединяют соседние грани, сокращая draw calls на 30-50 процентов. Статик-ассерты проверяют контракты на этапе компиляции. Передаю слово.»
