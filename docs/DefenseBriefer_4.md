...
> «Физика в ProjectV построена на библиотеке Jolt Physics — это современный движок для твёрдых тел, детерминированный, с поддержкой SIMD и многопоточности, лицензия MIT. Jolt используется для обнаружения столкновений и как прокси для контроллера персонажа, но авторитетным для игрока является собственный voxel-решатель — это зафиксировано в наших инженерных решениях.
>
...
>
> Walk-контроллер имеет несколько специальных возможностей. **Edge grace** — это когда игрок стоит на тонком крае, контроллер не дёргает его вверх-вниз при микро-перепаде высот. **Sneak** — при удержании Shift игрок не прилипает к стене при попытке зайти за угол. **Авто-прыжок** — при подходе к блоку высотой 1 игрок автоматически перепрыгивает, включается клавишей J, отключается F12. Контроль в воздухе — Minecraft-подобный по умолчанию, реалистичный с фиксацией направления.
>
...
| **CharacterVirtual** | Прокси-класс Jolt для персонажа. Collision detection, **не** авторитетный для grounded. |
| **Voxel solver** | Наш собственный решатель в `PhysicsWorld.cpp`, авторитетный для walk grounded. |
...
| **Edge grace** | Допуск тонкого края: контроллер не дёргает игрока вверх-вниз при микро-перепаде. |
...
> «Jolt `CharacterVirtual` авторизует grounded через форму коллизии (capsule/box), не знает про структуру вокселей. Для плоских поверхностей это OK, но в voxel-мире нужны edge grace (тонкие края), sneak (sampled top-plane), top-promotion. Voxel solver знает раскладку чанков и может давать эту семантику. CharacterVirtual остаётся как прокси/носитель стойки для collision detection. Зафиксировано в `decisions.md §6`.»

### Если спрашивают «что такое edge grace»:
> «Когда игрок стоит на краю блока, voxel'и под стопой могут быть чуть разной высоты (1/8 вокселя = 12.5 см). Без edge grace контроллер дёргал бы игрока вверх-вниз при micro-movement. С edge grace — допуск в 0.1 м, контроллер сохраняет текущую Y, не дёргает. Подобрано эмпирически.»
...
> «Эмпирический параметр в `PhysicsWorld.cpp`. Допуск в 0.1 м (1/10 вокселя). Когда разница между sampled top-plane и текущим Y игрока меньше edgeGraceThreshold, контроллер НЕ обновляет Y, сохраняет стабильность. Когда больше — apply Y delta. Подобрано так, чтобы микро-перепады на плотных сценах (1/8 вокселя = 12.5 см) не дёргали игрока.»
