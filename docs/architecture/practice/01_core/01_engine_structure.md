...
слоям с использованием **C++26 Modules** и **PIMPL паттерна** для изоляции сторонних библиотек.
...
│   ├── ProjectV.Core.Jobs.cppm         # Job system (std::execution)
│   └── ProjectV.Core.Platform.cppm     # Platform abstraction (SDL3)
│
├── render/
│   ├── ProjectV.Render.cppm            # Primary render module interface
│   ├── ProjectV.Render.Vulkan.cppm     # Vulkan 1.4 wrapper (Dynamic Rendering)
