...
**volk** и **Vulkan Memory Allocator (VMA)** — фундаментальные библиотеки для работы с Vulkan в ProjectV. Не абстракции, а инструменты для прямого управления железом.
...
**Метафора**: VMA — это "менеджер склада", который эффективно использует каждый квадратный метр, вместо того чтобы арендовать отдельные комнаты для каждой коробки.
...
- [ProjectV Architecture: Memory Management](../../architecture/practice/04_memory.md)
