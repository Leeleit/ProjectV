...
**Фоллбеки** при сбое основного web_search (Exa): DuckDuckGo → Brave → Startpage → Searx → прямой `webfetch` к
каноничным URL. Если все недоступны — уведомить оператора.
...
- Синхронизация всех документов после каждого тика. Обновлять **только** то, что изменилось в рамках текущего
  эксперимента.
- Копирование больших блоков из `agent/knowledge.md` к себе. Только cross-refs (`agent/knowledge.md`, конкретные §-номера существующих секций).
