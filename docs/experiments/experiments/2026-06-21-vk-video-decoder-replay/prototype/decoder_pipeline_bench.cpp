...
constexpr const char* ScenarioName(Scenario s) {
    switch (s) {
        case Scenario::Intro_720p30: return "intro_720p30";
        case Scenario::Cinematic_1080p60: return "cinematic_1080p60";
        case Scenario::Replay_1080p60: return "replay_1080p60";
        case Scenario::Trailer_4K30: return "trailer_4k30";
    }
    return "?";
}
...
constexpr const char* BitrateName(Bitrate b) {
    switch (b) {
        case Bitrate::Low_2Mbps: return "2mbps";
        case Bitrate::High_8Mbps: return "8mbps";
    }
    return "?";
}
...
constexpr const char* StrategyName(Strategy s) {
    switch (s) {
        case Strategy::A_ExternalPlayer: return "A_ExternalPlayer";
        case Strategy::B_FFmpegSWDecoder: return "B_FFmpegSWDecoder";
        case Strategy::C_VulkanVideoHWDecoder: return "C_VulkanVideoHWDecoder";
    }
    return "?";
}
...
Resolution GetResolution(Scenario s) {
    switch (s) {
        case Scenario::Intro_720p30: return {1280, 720, 30};
        case Scenario::Cinematic_1080p60: return {1920, 1080, 60};
        case Scenario::Replay_1080p60: return {1920, 1080, 60};
        case Scenario::Trailer_4K30: return {3840, 2160, 30};
    }
    return {0, 0, 0};
}
...
double SWDecoderBase1080pUs(Codec c) {
    switch (c) {
        case Codec::H264: return 3500.0;
        case Codec::H265: return 5000.0;
        case Codec::AV1: return 8000.0;
    }
    return 0.0;
}
...
double HWDecoderBase1080pUs(Codec c) {
    switch (c) {
        case Codec::H264: return 100.0;
        case Codec::H265: return 80.0;
        case Codec::AV1: return 150.0;
    }
    return 0.0;
}
...
double ScenarioComplexityMultiplier(Scenario s) {
    switch (s) {
        case Scenario::Intro_720p30: return 0.8;
        case Scenario::Cinematic_1080p60: return 1.0;
        case Scenario::Replay_1080p60: return 1.2;
        case Scenario::Trailer_4K30: return 2.0;
    }
    return 1.0;
}
...
double BitrateComplexityMultiplier(Bitrate b) {
    switch (b) {
        case Bitrate::Low_2Mbps: return 0.7;
        case Bitrate::High_8Mbps: return 1.3;
    }
    return 1.0;
}
...
double DPBMemoryMB(Scenario s) {
    ...
    return (static_cast<double>(kDPBFrames) * static_cast<double>(res.width) *
            static_cast<double>(res.height) * kBytesPerPixel420) / (1024.0 * 1024.0);
}
...
FrameCost ComputeCost(Strategy strategy, Scenario s, Bitrate b, Codec codec) {
    switch (strategy) {
        case Strategy::A_ExternalPlayer: return A_Cost(s, b);
        case Strategy::B_FFmpegSWDecoder: return B_Cost(s, b, codec);
        case Strategy::C_VulkanVideoHWDecoder: return C_Cost(s, b, codec);
    }
    return {};
}
...
ConfigResult RunConfig(Strategy strategy, Scenario s, Bitrate b, Codec codec, int seed,
    ...
    if (cost.cpu_time_us > 1000.0) stddev_pct = 0.10;
    ...
    for (size_t i = 0; i < cpu_samples.size(); ++i) {
        ...
        if (i == 0) total += cost.first_frame_latency_us;
...
int Main() {
    ...
    for (const auto& [strategy, configs] : by_strategy) {
        ...
        for (const auto& c : configs) means.push_back(c.mean_total_us);
        std::vector<double> p99s;
        for (const auto& c : configs) p99s.push_back(c.p99_total_us);
        Stats s_mean = Compute(means);
        Stats s_p99 = Compute(p99s);
        double mean_vram = 0;
        for (const auto& c : configs) mean_vram += c.peak_vram_mb;
