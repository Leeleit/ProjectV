...
static GameState make_scene(int scene_id, int seed) {
    uint64_t s = 0xDEADBEEFCAFEULL + static_cast<uint64_t>(scene_id) * 1000ULL
    ...
    case 0: // early_war_breakthrough: high initial unit strength, low stockpile, neutral relations
        for (int i = 0; i < 5; ++i) g.stockpile[i] *= 0.5f;
        for (int i = 0; i < 5; ++i) g.faction_relations[i] = 0.0f;
        break;
    ...
    case 2: // late_war_attrition: depleted stockpile, low relations
        for (int i = 0; i < 5; ++i) g.stockpile[i] *= 0.3f;
        for (int i = 0; i < 5; ++i) g.faction_relations[i] = -0.7f;
        break;
    case 3: // naval_invasion: theaters 0-2 are coastal, 3-7 are inland
        for (int i = 0; i < 3; ++i) g.theater_threat[i] = 0.8f;
        for (int i = 3; i < 8; ++i) g.theater_threat[i] = 0.3f;
        break;
    case 4: // defensive_counterattack: enemy in theaters 0-2, friendly weak
        ...
        for (int i = 3; i < 8; ++i) g.unit_strength[i][0] = std::min(1.0f, g.unit_strength[i][0] * 1.5f);
        break;
    }
    return g;
}
...
static const std::array<StrategyConfig, 5>& strategies() {
    static const std::array<StrategyConfig, 5> cfgs = {{
        ...
        {0, "A_HeuristicWeightedScore", 0.50,
            {0.50, 0.50, 0.50, 0.50, 0.50},
            0.1, 0, 0},
        // B: RAG_StrategicDoc
        {1, "B_RAG_StrategicDoc", 0.65,
            {0.67, 0.65, 0.75, 0.65, 0.65},
            1500.0, 3000, 1},
        // C: HierarchicalStrategicTactical (IFPV pattern)
        {2, "C_HierarchicalStrategicTactical", 0.74,
            {0.84, 0.82, 0.79, 0.82, 0.81},
            2500.0, 4500, 5},
        // D: ReActPlanExecute
        {3, "D_ReActPlanExecute", 0.72,
            {0.77, 0.77, 0.72, 0.77, 0.82},
            3000.0, 4000, 8},
        // E: PureTactical_2Hz (no strategic layer)
        {4, "E_PureTactical_2Hz", 0.55,
            {0.50, 0.55, 0.50, 0.55, 0.55},
            2000.0, 2000, 4},
    }};
    return cfgs;
}
...
static StrategicPlan generate_plan(const GameState& state, int strategy_id, int seed) {
    StrategicPlan p{};
    uint64_t s = hash_state(state) + static_cast<uint64_t>(strategy_id) * 999983ULL
                 + static_cast<uint64_t>(seed) * 7919ULL;
    ...
    for (int i = 0; i < 8; ++i) total_threat += state.theater_threat[i];
    if (total_threat < 1e-3f) total_threat = 1.0f;
    ...
    for (int i = 0; i < 8; ++i) {
        ...
        float p_alloc = (strategic_awareness[strategy_id] * state.theater_threat[i]
                         + (1.0f - strategic_awareness[strategy_id]) * (1.0f / 8.0f));
        for (int j = 0; j < 6; ++j) {
    ...
    for (int i = 0; i < 3; ++i) {
        p.production_allocation[i] = prod_bias[strategy_id] * state.stockpile[i]
                                   + (1.0f - prod_bias[strategy_id]) * hash_to_unit(splitmix64(s));
...
static double evaluate_plan(const GameState& state, const StrategicPlan& plan) {
    ...
    for (int i = 0; i < 8; ++i) {
        ...
        for (int j = 0; j < 6; ++j) alloc_sum += plan.unit_allocation[i][j];
        threat_coverage += state.theater_threat[i] * std::min(1.0f, alloc_sum);
        total_threat += state.theater_threat[i];
    }
    if (total_threat > 1e-6) threat_coverage /= total_threat;
    else threat_coverage = 1.0;

    double mean_s = 0.0;
    for (int i = 0; i < 5; ++i) mean_s += state.stockpile[i];
    mean_s /= 5.0;
    double var_s = 0.0;
    for (int i = 0; i < 5; ++i) var_s += (state.stockpile[i] - mean_s) * (state.stockpile[i] - mean_s);
    var_s /= 5.0;
    ...
    for (int i = 0; i < 8; ++i) {
        for (int j = 0; j < 6; ++j) {
            if (plan.unit_allocation[i][j] > max_alloc) max_alloc = plan.unit_allocation[i][j];
        }
    }
    ...
    // Weighted sum.
    return 0.35 * threat_coverage
         + 0.15 * stockpile_efficiency
         + 0.15 * diplomacy_rationality
         + 0.20 * prod_match
         + 0.10 * alloc_coherence
         + 0.05 * tool_eff;
}
...
static bool check_coherence(const GameState& state, const StrategicPlan& plan) {
    ...
    for (int i = 0; i < 8; ++i) {
        float sum = 0.0f;
        for (int j = 0; j < 6; ++j) sum += plan.unit_allocation[i][j];
        if (sum > 6.0f) return false;
    }
    return true;
}
        ...
        for (int sc = 0; sc < kNumScenes; ++sc) {
            for (int sd = 0; sd < kNumSeeds; ++sd) {
                ...
                for (int w = 0; w < kWarmup; ++w) {
                    volatile auto _ = run_decision(si, sc, sd); (void)_;
                }
                ...
                for (int it = 0; it < kMain; ++it) {
                    ...
                    if (m.coherence_pass) ++coherence_count;
