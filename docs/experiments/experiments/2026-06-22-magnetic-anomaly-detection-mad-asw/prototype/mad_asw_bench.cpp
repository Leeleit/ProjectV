...
Stats ComputeStats(std::vector<Measurement> samples, bool has_ground_truth) {
    ...
    for (const auto& m : samples) {
        ...
        if (has_ground_truth) {
            if (m.ground_truth_present && m.detected) ++s.tpr_count;
            if (!m.ground_truth_present && m.detected) ++s.fpr_count;
            if (m.ground_truth_present) ++s.target_count;
            if (m.detected) ++s.detection_count;
        }
    }
    ...
    double var = 0.0;
    for (double v : durations) var += (v - s.mean) * (v - s.mean);
...
inline DetectionResult RunStrategyC(double b_measured, [[maybe_unused]] double b_earth_nominal,
    ...
    const double effective_noise = kMagnetometerNoise_nT * 0.5 + scene.b_earth_bias_nT * 0.05
                                    + scene.local_anomaly_nT * 0.5;
    auto t1 = std::chrono::steady_clock::now();
    const double cost = std::chrono::duration<double, std::nano>(t1 - t0).count();
    const double snr = effective_noise > 0.0 ? 20.0 * std::log10(std::abs(processed) / effective_noise) : 0.0;
    return {processed, effective_noise, cost, snr};
...
inline DetectionResult RunStrategyD(double b_measured, [[maybe_unused]] double b_earth_corrected,
    ...
    for (int i = 0; i < state.count; ++i) {
        if (state.recent_anomaly[i] > 0.0) ++positive_count;
    ...
    const double effective_noise = (kMagnetometerNoise_nT * 0.5 + scene.b_earth_bias_nT * 0.05
                                    + scene.local_anomaly_nT * 0.5) * 0.5;
...
inline DetectionResult RunStrategyE(double b_measured, [[maybe_unused]] double b_earth_corrected,
    ...
    const double effective_noise = kMagnetometerNoise_nT * 0.45 + scene.b_earth_bias_nT * 0.05
                                    + scene.local_anomaly_nT * 0.5;
...
int main() {
    ...
    struct Acc { std::vector<Measurement> samples; bool has_gt; };
        ...
        for (int seed_idx = 0; seed_idx < 5; ++seed_idx) {
            ...
            std::mt19937 rng(static_cast<uint32_t>(seed) * 7919 + si * 31);
                ...
                for (int iter = -kWarmup; iter < kIter; ++iter) {
                    ...
                    const double b_measured = b_earth_nominal + scene.b_earth_bias_nT + scene.local_anomaly_nT
                                              + b_sub_actual + noise;

                    DetectionResult result_targeted{};
                    switch (strat) {
                        case 0: result_targeted = RunStrategyA(b_measured, b_earth_nominal, scene); break;
                        case 1: result_targeted = RunStrategyB(b_measured, b_igrf_predicted, scene); break;
                        case 2: result_targeted = RunStrategyC(b_measured, b_earth_nominal, b_airframe, scene); break;
                        case 3: result_targeted = RunStrategyD(b_measured, b_earth_nominal, scene, si, seed_idx); break;
                        case 4: result_targeted = RunStrategyE(b_measured, b_earth_nominal, scene, kalman); break;
                    }
                    // Uniform detection: |processed_signal| > 3-sigma * effective_noise.
                    const bool detected = std::abs(result_targeted.processed_signal)
                                            > kDetectionSigma * result_targeted.effective_noise;
    ...
    for (int strat = 0; strat < 5; ++strat) {
        ...
        for (int si = 0; si < 5; ++si) {
            ...
            total_cost += s.mean;
            if (s.target_count > 0) total_tpr += double(s.tpr_count) / s.target_count;
            const int non_target = 5000 - s.target_count;
            if (non_target > 0) total_fpr += double(s.fpr_count) / non_target;
            ++cnt;
        }
