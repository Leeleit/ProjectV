...
double synthetic_gpu_work_us(int ray_march_count) {
    ...
    for (int r = 0; r < ray_march_count; ++r) {
        for (int i = 0; i < kRayMarchIters; ++i) {
            ...
            if (cell < 128) local_sink += std::sqrt(v + 1.0);
            else if (cell < 192) local_sink += std::sin(v * 3.14159);
            else local_sink += std::cos(v * 6.28318);
...
enum class PresentMode { Local, LocalMultiDevice, Sum, Remote };
...
double simulate_frame_interval_us(PresentMode mode, const Interconnect& ic, int gpu_count, int work_size, int /*iter*/) {
    ...
    switch (mode) {
        case PresentMode::Local: {
            if (gpu_count == 1) return single_gpu_work_us;
        ...
        case PresentMode::Sum: {
            ...
            if (gpu_count < 2) return single_gpu_work_us;
...
int main(int argc, char** argv) {
    ...
    if (argc > 1) {
        for (int i = 1; i < argc; i += 2) {
            ...
            if (arg == "--output" && i + 1 < argc) output_path = argv[i + 1];
            else if (arg == "--warmup" && i + 1 < argc) warmup = std::atoi(argv[i + 1]);
            else if (arg == "--iter" && i + 1 < argc) iters = std::atoi(argv[i + 1]);
        }
    }
    ...
    for (const auto& row : rows) {
        for (size_t i = 0; i < row.size(); ++i) {
            if (i > 0) f << ',';
            if (row[i].find(',') != std::string::npos) f << '"' << row[i] << '"';
            else f << row[i];
        }
        f << '\n';
    }
