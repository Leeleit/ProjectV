...
struct VoxelChunk {
    ...
    void randomize(uint64_t seed) {
        ...
        for (auto& v : voxels) v = static_cast<uint8_t>(dist(rng));
    }
};
...
CostResult computeFrameCost(int strategy_idx, const Scene& scene, uint64_t seed) {
    ...
    if (strategy_idx == 1) {
        ...
        const double sort_ms = (kRtx3090SortMsPerMSplatsMin + kRtx3090SortMsPerMSplatsMax) / 2.0
            * splat_factor * kRtx3060TiSlowdownVsRtx3090;
        const double raster_ms = (kRtx3090RasterMsPerMSplatsMin + kRtx3090RasterMsPerMSplatsMax) / 2.0
            * splat_factor * kRtx3060TiSlowdownVsRtx3090;
        ...
        r.vram_mb = (kSplatStrideBytes * scene.splat_count) / (1024.0 * 1024.0)
                  + 16.0 // framebuffers
                  + 8.0;  // sort buffers
        return r;
    }
    ...
    if (strategy_idx == 2) {
        ...
        r.vram_mb = (kSplatStrideBytes * scene.splat_count) / (1024.0 * 1024.0)
                  + 16.0 + 8.0
                  + (512.0 * scene.visible_voxel_chunks) / (1024.0 * 1024.0);
        return r;
    }
    ...
    if (strategy_idx == 3) {
        ...
        r.mean_mutation_ms = (kRetrainMsPerChunkEditMin + kRetrainMsPerChunkEditMax) / 2.0
                          * scene.chunks_per_edit;
        r.stale_splats = 0;
        r.vram_mb = (kSplatStrideBytes * scene.splat_count) / (1024.0 * 1024.0)
                  + 16.0 + 8.0
                  + 50.0; // retrain scratch buffer
        return r;
    }
...
BenchResult runBench(int strategy_idx, const std::string& strategy_name,
    ...
    double sq_sum = 0.0;
    for (double t : frame_times) sq_sum += (t - r.mean_frame_ms) * (t - r.mean_frame_ms);
