...
## Compute Shader Fallback (Обязательно!)

### Проблема совместимости

Mesh Shaders поддерживаются не на всех GPU. Для широкого распространения ProjectV **обязан** иметь fallback на
традиционный compute shader pipeline.

### GPU совместимость

| GPU                             | Mesh Shaders       | Fallback Required |
|---------------------------------|--------------------|-------------------|
| **NVIDIA RTX 20xx+ (Turing)**   | ✅                  | Нет               |
| **NVIDIA GTX 10xx (Pascal)**    | ❌                  | Да                |
| **AMD RDNA2+ (RX 6000)**        | ✅                  | Нет               |
| **AMD RDNA1 (RX 5000)**         | ❌                  | Да                |
| **AMD GCN (RX 500, Vega)**      | ❌                  | Да                |
| **Intel Arc (Alchemist)**       | ✅                  | Нет               |
| **Intel Integrated (UHD/Iris)** | ❌                  | Да                |
| **Apple M1/M2 (Metal)**         | ❌ (через MoltenVK) | Да                |

### Проверка поддержки

```cpp
bool checkMeshShaderSupport(VkPhysicalDevice physicalDevice) {
    VkPhysicalDeviceMeshShaderFeaturesEXT meshFeatures = {
        .sType = VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_MESH_SHADER_FEATURES_EXT
    };
...
enum class GeometryPipeline {
    MeshShader,      // Предпочтительный путь (Turing+, RDNA2+)
    ComputeVertex,   // Fallback: Compute генерация + Vertex shader
    Traditional      // Ultimate fallback: CPU генерация
};

class VoxelGeometryPipeline {
private:
    GeometryPipeline activePipeline_;
    VkPipeline meshShaderPipeline_;
    VkPipeline computeVertexPipeline_;
    VkPipeline traditionalPipeline_;

public:
    VoxelGeometryPipeline(VkDevice device, VkPhysicalDevice physicalDevice) {
        if (checkMeshShaderSupport(physicalDevice)) {
            activePipeline_ = GeometryPipeline::MeshShader;
            meshShaderPipeline_ = createMeshShaderPipeline(device);
        } else {
            activePipeline_ = GeometryPipeline::ComputeVertex;
            computeVertexPipeline_ = createComputeVertexPipeline(device);
        }

        // Всегда создаём traditional pipeline как ultimate fallback
        traditionalPipeline_ = createTraditionalPipeline(device);
    }

    void render(VkCommandBuffer cmd, const VoxelChunk& chunk) {
        switch (activePipeline_) {
            case GeometryPipeline::MeshShader:
                renderMeshShader(cmd, chunk);
                break;
            case GeometryPipeline::ComputeVertex:
                renderComputeVertex(cmd, chunk);
                break;
            case GeometryPipeline::Traditional:
                renderTraditional(cmd, chunk);
                break;
        }
    }

private:
    void renderMeshShader(VkCommandBuffer cmd, const VoxelChunk& chunk) {
        vkCmdBindPipeline(cmd, VK_PIPELINE_BIND_POINT_GRAPHICS, meshShaderPipeline_);
        // Mesh shader рендеринг без vertex buffer
        vkCmdDrawMeshTasksEXT(cmd, chunk.groupCountX, chunk.groupCountY, chunk.groupCountZ);
    }

    void renderComputeVertex(VkCommandBuffer cmd, const VoxelChunk& chunk) {
        // Pass 1: Compute shader генерирует вершины
        vkCmdBindPipeline(cmd, VK_PIPELINE_BIND_POINT_COMPUTE, computePipeline_);
        vkCmdDispatch(cmd, chunk.groupCountX, chunk.groupCountY, chunk.groupCountZ);

        // Memory barrier между compute и graphics
        ...
        // Pass 2: Traditional vertex shader рендеринг
        vkCmdBindPipeline(cmd, VK_PIPELINE_BIND_POINT_GRAPHICS, vertexPipeline_);
        vkCmdDrawIndirect(cmd, indirectBuffer, 0, drawCount, sizeof(VkDrawIndirectCommand));
    }
};
```

### Compute Shader для генерации геометрии

```glsl
// fallback_voxel_gen.comp
#version 460
...
uint emitVertex(vec3 pos, vec3 normal, uint material) {
    uint idx = atomicAdd(vertexCount, 1);
    vertices[idx] = Vertex(pos, normal, material);
    return idx;
}

void emitQuad(vec3 base, vec3 normal, uint material, uint axis) {
    // Генерация 6 вершин (2 треугольника) для одной грани
    vec3 v0 = base;
    vec3 v1, v2, v3;

    if (axis == 0) { // X face
        v1 = base + vec3(0, voxelSize, 0);
        v2 = base + vec3(0, 0, voxelSize);
        v3 = base + vec3(0, voxelSize, voxelSize);
    } else if (axis == 1) { // Y face
        v1 = base + vec3(voxelSize, 0, 0);
        v2 = base + vec3(0, 0, voxelSize);
        v3 = base + vec3(voxelSize, 0, voxelSize);
    } else { // Z face
        v1 = base + vec3(voxelSize, 0, 0);
        v2 = base + vec3(0, voxelSize, 0);
        v3 = base + vec3(voxelSize, voxelSize, 0);
    }

    emitVertex(v0, normal, material);
    emitVertex(v1, normal, material);
    emitVertex(v2, normal, material);
    emitVertex(v1, normal, material);
    emitVertex(v3, normal, material);
    emitVertex(v2, normal, material);
}
...
void main() {
    ...
    uint index = coord.x + coord.y * CHUNK_SIZE + coord.z * CHUNK_SIZE * CHUNK_SIZE;
    uint voxel = voxels[index];

    if (voxel == 0) return;  // Air

    uint material = voxel & 0xFF;

    // Проверка соседей и генерация только видимых граней
    vec3 worldPos = (chunkOffset + coord) * voxelSize;

    // Проверка X-
    if (coord.x == 0 || voxels[index - 1] == 0) {
        emitQuad(worldPos, vec3(-1, 0, 0), material, 0);
    }
    // Проверка X+
    if (coord.x == CHUNK_SIZE - 1 || voxels[index + 1] == 0) {
        emitQuad(worldPos + vec3(voxelSize, 0, 0), vec3(1, 0, 0), material, 0);
    }
...
### Vertex Shader для Fallback

```glsl
// fallback_vertex.vert
#version 460
...
layout(location = 1) out flat uint fragMaterialIndex;
...
1. **Всегда реализуйте fallback** — Mesh Shaders не гарантированы
2. **Тестируйте на разных GPU** — Проверяйте fallback на старом железе
3. **Динамическое переключение** — Автоматически выбирайте лучший доступный pipeline
4. **Одинаковый вывод** — Результат должен быть идентичен независимо от pipeline
