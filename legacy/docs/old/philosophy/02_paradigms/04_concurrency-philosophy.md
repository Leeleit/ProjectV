# Философия Конкурентности (Concurrency)

В 2026 году процессор — это не одно быстрое ядро, а 16-32 "медленных". Чтобы утилизировать их все, нам нужна правильная
модель.
**`std::thread` — это для ОС. Fiber — это для движка.**

---

## Проблема с `std::thread`

Создание потока ОС — это дорого (сискол, аллокация стека 1MB+). Переключение контекста (context switch) — это катастрофа
для кэша (тысячи циклов).
Если у вас 1000 задач, создавать 1000 потоков — самоубийство.
...
> потоками ещё дороже: нужно сохранить все регистры, переключить контекст, обновить TLB (кэш адресов). В процессе
> теряется
> кэш L1/L2. Для сервера с миллисекундными задержками это норм. Для игры с 16.6ms на кадр — это смерть.
...
* **M Fibers (Jobs):** Легковесные задачи с крошечным стеком (64KB). Их тысячи.
...
> свой кабинет (1 MB стека), его нужно оформить в HR (syscall) — это дорого и долго. К тому же, мастеров некуда
> посадить —
> мест 16.
>
...
1. Вы создаете задачу (Job): "Обновить физику для чанка #42".
2. Задача кладется в очередь (Global Queue или Thread-Local Deque).
3. Свободный Worker берет задачу и выполняет её.
4. Если задача ждет другую задачу (Dependency), текущий файбер "засыпает" (yield), но поток **не блокируется**. Он берет
   другую задачу.
...
Мьютексы (`std::mutex`) — это зло. Они заставляют поток спать (syscall), убивая производительность.
В Job System мы используем:

1. **Work Stealing Deques:** Каждый воркер имеет свою очередь. Если она пуста, он "крадет" задачу у соседа. (C++
   `std::atomic` + CAS).
...
> ничего не делают. В Job System мы не ждём. Если туалет занят — идём работать в другой комнате. Work Stealing — это
> когда
...
> параллельно. Физика и AI не зависят друг от друга — значит, выполняются одновременно на разных ядрах. Это
> автоматический
...
В такой системе Data Race — главный враг.
Мы решаем это архитектурно:
...
> записи. Никаких гонок. Partitioning: каждый поток работает со своим куском данных. Read-Only: если никто не пишет,
> гонки
> не бывает.
