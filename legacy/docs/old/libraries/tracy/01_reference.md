...
```cpp
// Tracy поддерживает профилирование блокировок, но в ProjectV используйте альтернативы:
// 1. Lock-free структуры (атомарные операции)
// 2. stdexec sender'ы для асинхронной обработки
// 3. Thread-local данные

// Пример lock-free подхода в ProjectV:
// std::atomic<int> counter{0};
// counter.fetch_add(1, std::memory_order_relaxed);

// Если необходимо профилировать contention, используйте Tracy для атомарных операций:
// TracyPlot("LockFreeCounter", counter.load(std::memory_order_relaxed));
```

**Примечание:** Tracy может профилировать contention даже в lock-free структурах через `TracyPlot` для отслеживания
состояния атомарных переменных.
