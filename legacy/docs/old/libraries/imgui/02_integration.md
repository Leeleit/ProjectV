...
> **Для понимания:** ImGui — это Immediate Mode UI библиотека. Представьте, что вы "рисуете" интерфейс каждый кадр:
> сказали `Button("Click")` — кнопка появилась, не сказали — её нет. В ProjectV мы интегрируем ImGui через MemoryManager
> для аллокаций, Log для ошибок и Tracy для профилирования.
