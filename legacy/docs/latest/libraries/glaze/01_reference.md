...
```cpp
// Пример воксельной структуры для Vulkan-ориентированного движка
struct VoxelChunk {
    glm::ivec3 coord;
...
### Производительность (усреднённые бенчмарки для Vulkan-ориентированного движка)
...
## Технические детали для Vulkan-ориентированного движка
...
## Почему это DOD-путь для Vulkan-ориентированного движка

### Память для воксельных данных

| Библиотека    | Паттерн памяти для вокселей   | Проблема для Vulkan-ориентированного движка |
|---------------|-------------------------------|---------------------------------------------|
...
## Стандарты интеграции для Vulkan-ориентированного движка

### C++26 Module с Global Module Fragment

```cpp
module Engine.Serialization.Glaze;
...
#include <engine/profiling/tracy.hpp>
export module Engine.Serialization.Glaze;
```
...
class GlazeAllocator {
    engine::core::memory::ArenaAllocator& arena_;

public:
    GlazeAllocator(engine::core::memory::ArenaAllocator& arena) : arena_(arena) {}
...
### Logging Integration через макросы
...
std::expected<T, std::string> load_voxel_config(const std::filesystem::path& path) {
    ...
    if (!std::filesystem::exists(path)) {
        LOG_ERROR("Voxel config file not found: {}", path.string());
    ...
    if (error) {
        LOG_ERROR("Failed to parse voxel config: {} at byte {}",
                  glz::format_error(error, path.string()), error.location);
        return std::unexpected("Parse error");
    }

    LOG_INFO("Loaded voxel config from: {}", path.string());
    return config;
}
```
...
public:
    ...
    static void profile_deserialize(std::string_view operation, size_t data_size) {
        PROFILE_SCOPE("GlazeDeserialize");
        TracyPlot("Glaze/DataSize", static_cast<int64_t>(data_size));
        TracyMessage(operation.data(), operation.size());
    }
};
...
## Ограничения в контексте Vulkan-ориентированного движка

1. **Требует C++20/23** — Vulkan-ориентированный движок использует C++26, поэтому совместимо
...
Glaze предоставляет для Vulkan-ориентированного движка идеальное решение для сериализации с фокусом на
производительность:
...
### Рекомендации по использованию в Vulkan-ориентированном движке:
