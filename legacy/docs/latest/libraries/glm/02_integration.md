# Интеграция GLM с Vulkan-ориентированным движком

## C++26 Module: Engine.Core.Math

GLM — header-only библиотека с огромным количеством шаблонов. Подключение через `#include` в каждый файл убивает
скорость компиляции. Решение: именованный модуль `Engine.Core.Math`.

### Структура модуля

```cpp
// Engine.Core.Math.ixx
module;
...
```cmake
# CMakeLists.txt для модуля Engine.Core.Math
add_library(engine_core_math INTERFACE)
target_include_directories(engine_core_math INTERFACE external/glm)

# Устанавливаем C++26 и модули
set_target_properties(engine_core_math PROPERTIES
  CXX_STANDARD 26
...
target_link_libraries(engine_core PRIVATE engine_core_math)
```
...
struct ChunkTransforms {
    ...
    alignas(64) std::vector<mat4> modelMatrices;
    alignas(64) std::vector<mat4> normalMatrices;
    alignas(64) std::vector<vec3> positions;
    alignas(64) std::vector<quat> rotations;
    alignas(64) std::vector<vec3> scales;

    ChunkTransforms(size_t capacity) {
...
// Генерация мешей чанков с использованием memory_resource
...
public:
    std::vector<vec3> generateGreedyMesh(
        ...
        std::pmr::monotonic_buffer_resource& tempArena
    ) {
        // Используем временную арену для промежуточных данных
        std::pmr::vector<vec3> vertices(&tempArena);
        std::pmr::vector<uint32_t> indices(&tempArena);

        // Генерация меша...
        // Все временные аллокации идут в tempArena
        // В конце кадра арена автоматически сбрасывается

        return std::vector<vec3>(vertices.begin(), vertices.end());  // Данные копируются в постоянное хранилище
    }
};
```
