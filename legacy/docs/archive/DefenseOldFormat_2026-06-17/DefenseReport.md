...
Полный per-commit авторский вклад — в `git log --author=` (см.
`legacy/docs/architecture/academic/01_project_defense_model.md`).
...
C/AVX2 ядром фрустум-кулинга и полным циклом разработки (сборка, тесты, профилирование, документация,
GitHub-репозиторий).
...
- Tier 0-5 закрыты (Vec3/Mat4, inplace_vector + StringID + std::expected, C++20 модули + libc++, C/AVX2 ядро, провод в
  движок, branch hints + EVIL docs + InputAction mask UB fix + benchmark)
