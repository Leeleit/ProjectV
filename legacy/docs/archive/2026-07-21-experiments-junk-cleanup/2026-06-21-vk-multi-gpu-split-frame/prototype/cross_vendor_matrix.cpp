...
int main(int argc, char **argv)
{
	...
	for (const auto &[ic, rows] : afr_2) {
		...
		if (ic.find("NVLink 4.1") != std::string::npos)
			note = "Blackwell B200, NVLink 4.1 — 14× PCIe 5.0";
		else if (ic.find("xGMI") != std::string::npos)
			note = "RDNA 3, xGMI 2.0 — ~6× PCIe 5.0";
		else if (ic.find("PCIe 4.0") != std::string::npos)
			note = "Intel Arc Battlemage, NO native peer interconnect";
		else if (ic.find("PCIe 5.0") != std::string::npos)
			note = "Consumer Blackwell, NO NVLink (consumer only)";
