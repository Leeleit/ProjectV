...
**Across 5 interconnects × 4 present modes × 3 GPU counts × 5 work sizes = 300 configs × 30 iterations = 9000
measurements.**
...
1. **AFR scales near-ideally on 2-GPU** (~2.13-2.35× across all tiers) — slightly above 2× because per-GPU work has
   fixed overhead that doesn't scale
2. **AFR super-linear on 4-GPU** (~3.83-4.10×) — constant present sync overhead (30-80 µs) + per-GPU 5% sync
   amortization
3. **Even slow PCIe 4.0 (32 GB/s) gives 3.83×** — because peer copy is only 4 MiB/frame, dwarfed by GPU work (~7 ms at
   work=4096 rays)
...
3. **SFR best for bandwidth-bound** sub-regions (VCT atlas, BLAS pool) where spatial division helps; **worse for
   balanced render**
...
1. **REMOTE scales ~2× max** — limited by max(compute, render) — compute on GPU 1 (40% of frame), render on GPU 0 (60%
   of frame), parallel
2. **Best for compute-heavy mixed workload** (Fluid CA 20 Hz + world gen async + VCT cone-march) where compute is
