...
- **4 strategies:** A_None (baseline uniform 1x1) / B_FixedFoveation2x (center 30% 1x1 + periphery 2x2, no gaze) /
  C_GazeFoveation2x (gaze-driven 1x1 + 2x2 + 4x4) / D_GazeFoveation4x (gaze-driven aggressive, same algorithm as C for
  clarity)
- **5 scenes:** uniform_floor / forest_floor / cave_stress / mixed_biome / uniform_air (per
  `2026-06-21-sub-chunk-layers` precedent for direct comparability)
...
Each viewport is divided into **16×16 pixel tiles** (matches Vulkan min tile size on most hardware). Each tile gets a
...
Real gaze input would come from `XR_EXT_eye_gaze_interaction` (OpenXR 1.0+ ratified rev 2, 2024). For this CPU prototype
we synthesize gaze position per-frame from a per-scene Gaussian distribution centered at (0.5, 0.5) with scene-dependent
σ:
...
The model does not require GPU dispatch. Cross-vendor savings depend on whether the hardware supports the relevant
density values. Per `NVK Mesa DeepWiki` and `NVIDIA Developer Vulkan Driver`:
...
- **Mobile (Arm Mali, Qualcomm Adreno):** supports via `VK_QCOM_fragment_density_map_offset` (Tile Offset per Meta Quest
  ETFR production)
...
- Per-fragment cost = constant (no ALU/memory simulation). Real cost depends on specific fragment shader (VCT vs TAA vs
  RTX shadow). For full cost analysis use the GPU prototype in `voxel.frag` (deferred to mainline integration).
