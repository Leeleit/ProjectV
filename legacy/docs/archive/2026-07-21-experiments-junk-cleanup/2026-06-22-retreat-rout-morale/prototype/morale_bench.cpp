...
double RunBreakpoint(std::vector<Unit> &units, float casualty_rate, float sup_rate, float dt)
{
	...
	for (auto &u : units) {
		...
		if (u.state == STEADY) {
			if (casualty_rate > 0.5f)
				u.state = SHAKEN;
			else if (sup_rate > 5.0f)
				u.state = SHAKEN;
			if (!u.leader_alive)
				u.state = BREAKING;
		} else if (u.state == SHAKEN) {
			if (casualty_rate > 1.0f)
				u.state = BREAKING;
			if (casualty_rate > 2.0f || sup_rate > 10.0f)
				u.state = ROUT;
			// Recovery
				...
				for (int cri = 0; cri < 5; ++cri) {
					for (int sdi = 0; sdi < 5; ++sdi) {
						...
						for (int i = 0; i < 1010; ++i) {
							...
							switch (si) {
							case 0:
								t = RunNaive(units, cas_rate, sup_rate, dt);
								break;
							case 1:
								t = RunSigmoid(units, cas_rate, sup_rate, dt);
								break;
							case 2:
								t = RunAccumulator(units, cas_rate, sup_rate, dt);
								break;
							case 3:
								t = RunBreakpoint(units, cas_rate, sup_rate, dt);
								break;
							case 4:
								t = RunHybrid(units, cas_rate, sup_rate, dt);
								break;
							}
							if (i >= 10)
								samples.push_back(t);
