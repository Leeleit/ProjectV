...
- Task 3.4 (RT pipeline libraries + deferred host ops):
  ...
  - Second attempt added per-stage Tracy zones (`CreateRayGenLibrary`, `CreateMissLibrary`, `CreateHitGroupLibrary`, `DeferredJoin`, `LinkLibraries`) and confirmed all three libraries were deferred (`rayGen=yes miss=yes hitGroup=yes`). Per-stage data: kickoffs ~0.2 ms each, joins ~10–16 ms in parallel, link ~0.35 ms, total main-thread `CreatePipeline` ~16.6 ms — still ~2× slower than monolithic. Archived the attempt at `legacy/docs/archive/2026-07-14-task34-attempt/` per the new AGENTS.md rollback rule, then rolled back to the monolithic path.
