...
- **`MusicState` enum: `Stopped | Playing | Paused`.** Three-valued. Pause = `stop` + forget cursor (see "no `ma_sound_set_time`" note below).
- **miniaudio 0.11+ has NO `ma_sound_set_time` API** (removed in 0.10+). v1 pause = stop + forget cursor, next play = restart from 0. The `m_pausedCursorMs` field is kept (and reset to 0 in `unloadCurrentTrack`) so v2 can re-introduce the cursor-save path without a stale-read hazard. v2 will need either a custom decoder wrapper or a different miniaudio API surface.
...
- **`MusicDirectoryPath` resolution: `PROJECTV_MUSIC_DIR` env → `./music` (CWD-relative) → `SDL_GetBasePath()/music` → fallback to `./music`.** The **CWD-relative `./music` is the primary fallback** (NOT `SDL_GetBasePath` like the screenshot/snapshot pattern) because the operator's "папка music в корне" intent is the repo root, and they run the binary from the repo root. The CWD-relative path is checked first via `std::filesystem::is_directory`; if `./music` exists on disk, it wins. The `SDL_GetBasePath` fallback is `build/.../bin/music/` — a weird place for user content — so it only kicks in if the CWD doesn't have a `music/` folder (e.g. operator runs from a scratch dir). Env-var override is the recommended path when neither works. **`is_directory` is the discriminator**, not just `exists()` — an empty `music/` directory the engine just created still counts as "exists" and the engine will scan it (finding 0 tracks), which is the correct user-facing behavior ("you have 0 tracks").
