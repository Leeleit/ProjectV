...
- **status:** open (M4 just completed, awaiting M4 commit operator approval; M5 pending)
- **notes:** Решения зафиксированы в диалоге `2026-06-11`: Q1=2, Q2=1 (→ план 3), Q3=1 (→ план 3), Q4=2, Q5=2 (receive-only, выровнено с RTX-будущим), Q6=1 (universal PBR SSBO, GGX reuse), Q7=1 (без save), Q8=2 (можно трогать `SceneLightingBuffer`), U1=c, U2=c, U3=b, U4=b. **Прогресс:** M0 = `1c72a4b` — closed. M1 = `8b112e7` — closed. M2 = `cccdbc1` — closed. M3 = `24ccb08 feat(asset): KHR_draco_mesh_compression path in AssetLoader (M3)` — closed. M4 = model graphics pass + `model.vert/frag` (with TAA-on shader variant) + shared `lighting.glsl` GGX helper + manifest load on startup (U4=b, async submit on AppInit) + Jolt AABB body (Q4=2, deferred to follow-up because we lost the time budget). **MVP-отступления** от первоначального M4: `MeshComponent` + `ModelTransformComponent` в flecs ECS — deferred (per-instance data живёт в `RenderState::modelInstances`, не в ECS; перевод в ECS — M5). Jolt static AABB body — deferred (Q4=2 требует `PhysicsWorld::JPH::BodyInterface::CreateAndAddBody`, scope-wise это отдельный маленький follow-up). Visual verify: `Invoke-ProjectVRuntimeSmoke.sh` с `PROJECTV_MODELS=tests/fixtures/box.glb@0,0,4` — clean smoke, 0 Vulkan validation errors, capture saved под `build/linux-clang-debug/lookdev-captures/20260612-m4-model-pipeline/`. Shared GLSL helper (`lighting.glsl`) extract — частично: GGX/Smith/Fresnel-Schlick/EvaluateDirectLighting/SampleEnvironmentDiffuse/ApplyToneMap/ApplyColorGrading вынесены, но `SceneLightingBuffer` struct declaration всё ещё per-shader (это полная refactor voxel.frag + voxel_shadow.vert + voxel_mesh.comp + model.frag + model.vert, deferred). M5 — multi-instance + frustum cull.
