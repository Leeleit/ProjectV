...
- **notes:** Subtask 1+2 work functionally complete: build green, ctest 1/1, smoke 6/6 with `PROJECTV_ENABLE_VALIDATION=ON` — 0 VUIDs / 0 Unfreed / 0 taaResolvePipeline errors. Source-of-truth для плана — `agent/memory.md` §10.12, §10.13. **Согласование с параллельной сессией `2026-06-11-asset-pipeline-m0-m5`:** asset-pipeline M0 (CMake wiring) уже в дереве рядом с моими TAA-изменениями в `CMakeLists.txt` / `src/CMakeLists.txt` — непересекающиеся правки. M4 (asset-pipeline) тоже планирует править `Renderer.cpp` / `core/Types.hpp` / `SceneResources.cpp` для `RecordModelCommands` и `ModelRenderState` — это прямой конфликт с моими TAA-изменениями, когда M4 начнётся. Решение — за оператором (см. диалог 22:14 UTC): коммитить сейчас (asset-pipeline будет rebase) или ждать M0-M3, чтобы не пересекаться.
