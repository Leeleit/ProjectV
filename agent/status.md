...
Updated: `2026-06-20` — **Dead ray-march compute pass removed (pet-project cleanup, r0)** (session `session-2026-06-20T-raymarch-stub-removal-r0`, pre-commit). Per pet-project directive 2026-06-20: defense stub (ТЗ п. 4.1.2, 2026-06-13) with broken `SetRayMarchEnabled` toggle (`if (isEnabled) return;` блокирует OFF), shader компилировался в `.spv` но никогда не диспатчился в `Renderer::RecordGraphicsCommands`, `SDLK_2` hotkey только менял флаг с 0 visual effect, `RequestRayMarchPipelineRecreate()` указывал на несуществующий pipeline. Удалено: `src/render/RayMarchPass.{hpp,cpp}` (65 lines) + `src/shaders/ray_march.comp` (129 lines) + 2 строки в `src/CMakeLists.txt` + 4 строки в `src/app/main.cpp` (include, recreate call, `SDLK_2` branch). **Net −185 lines**, build green, ctest 16/16 passed (0.82s, baseline parity). Future SVO/re-fraction/volumetric reimplements DDA from scratch (current shader used flat 3D-grid payload, не SVO). `decisions.md §31` фиксирует решение, `CHANGELOG.md` 2026-06-20 **Removed** секция.
