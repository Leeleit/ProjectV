...
Updated: `2026-06-20` — **TODO.md rewrite: dependency-aware 6-Stage GPU-driven roadmap v1 (closed in `6709ca9`)** (session `session-2026-06-20T-todo-rewrite-v1-r0`). Supersedes Tier 0..5 r0 (`2026-06-13`). Per operator: «требуется обновить TODO, переписать, основываясь на плане, который сгенерировал другой агент» + later «доделать TODO, исходя из советов другого агента» (dependency-mismatch analysis). New TODO = 299 lines, 6 Stages dependency-aware. **Critical reordering**: Stage 1 (new voxel storage: Sparse 64-trees + SVDAG + async audio) MUST land before any Stage 2-5 work — all Stage 2-5 GPU geometry/cull/sim/GI/LOD code reads from SVDAG. Stage 6 (Flecs ECS) moved from tech-debt backlog to mainline-and-parallel. Pre-Stage 0 quick wins (B1 redundant model load loop, B2 pipeline destroy on resize, B3 getenv per frame, B4 verify VSync closed). Stage 0 architectural decisions (A1 Vulkan 1.4→1.3, A2 Fluid CA reversal). Stage 1 (1.1 Sparse 64-trees, 1.2 SVDAG, 1.3 async audio scanPlaylist). Stage 2 (2.1 Mesh+Task shaders for SVDAG, 2.2 HZB cull, 2.3 Virtual Texturing). Stage 3 (3.1 GPU Fluid CA per §30.4, 3.2 incremental Jolt, 3.3 greedy physics meshing). Stage 4 (4.1 GPU noise, 4.2 geometry LOD, 4.3 lift draw distance cap). Stage 5 (5.1 VCT, 5.2 RTX shadows feature-flag, 5.3 TAA + motion vectors). Stage 6 (6.1 Flecs ECS migration parallel, 6.2 AppState PIMPL + std::span + r0 carry-overs). Cross-cutting **Verification policy**: A/B test buffers during data-format migrations, MeshingStress 5% threshold for adoption, inspected runtime captures for rendering close-out. Old TODO verbatim copy → `legacy/docs/archive/agent-todos/2026-06-tier-0-5-r0.md` (207 lines, `SUPERSEDED 2026-06-20` header). `agent/decisions.md §29` + `§30` headers get OUTDATED markers with new dependency-aware cross-refs. New `decisions.md §30.4` «GPU Fluid CA contract» (ping-pong compute + atomicOr + active chunk list + multi-tile determinism) per operator reversal. Mega-atomic single commit (6 files, +620/-152, 1 new), docs-only per `AGENTS.md §7.3.1`, no build verify needed. Safety-net `/tmp/before_todo_rewrite_v1_20260620T102935Z.patch` (0 bytes — clean tree before work, deleted per §7). Pre-commit «Commit?» → operator «Коммит» → commit `6709ca9`.

Updated: `2026-06-20` — **Tier 2 mainline C++20 modules + `import std;` blocked (r0)** (session `session-2026-06-20T-tier2-mainline-modules-r0`). Mega-atomic commit: 2 новых модуля (`src/core/Types.ixx`, `src/ecs/EcsWorld.ixx`) + 24 mainline .cpp/.hpp мигрированы на `import projectv.math;` / `import projectv.string_id;` + 5 tests мигрированы + CMake FILE_SET CXX_MODULES wiring + `core/Types.hpp` switched to `import` для downstream consumers. **Build time**: incremental через touch `Math.hpp`/`StringId.hpp` = **0.10 sec (190× vs baseline 18.93s)** — ни один mainline файл больше не `#include`-ит fallback headers. Cold rebuild ProjectV = 102.61s (с module BMI generation). ctest 16/16 passed (0.77s). **`import std;` в mainline ЗАБЛОКИРОВАН**: libc++ 22 std.cppm BMI конфликтует с transitive `<string>`/`<vector>` через `fmt/format.h` (используется ~70% mainline) — clang "redefinition of concept '__concat_indirectly_readable'". Probe-only в `tests/StdModuleProbe.cpp` работает. **Workaround для Ninja 1.13 dep-scan race**: первый build sequential `-j 1`, последующие параллельные. TODO.md Tier 2.A/B/D/E/G marked done; 2.C marked blocked with rationale в `decisions.md §30`. **Next:** Tier 5.6 AppState PIMPL, Tier 2 follow-ups (`import std;` deferred), или другие Tier'ы.

Updated: `2026-06-20` — **Tier 5.7 + 5.4 + 5.2 + 5.10 batch r0** (session `session-2026-06-20T-tier510-hot-invariant-tests-r0`, commit `72eca66`). Combined atomic commit covering 4 Tier 5 sub-tasks: (a) Tier 5.7 — 6 mirror helper functions extracted from `UpdateApp` (173-line block → 16-line caller, net -157 lines in mirror block + 193 lines of helpers); (b) Tier 5.2 — 8 `[[assume]]` annotations в SceneResources.hpp + Camera.cpp hot loops (`nearPlane >= 0`, `tanHalfFov >= 0`, `chunkExtentAndNonAir[3] > 0`, `extent.{width,height} > 0`); (c) Tier 5.4 audit — cpp files уже документированы через `k*`-prefixed constexpr constants, shader magic numbers out of scope per AGENTS.md §6.5; (d) Tier 5.10 — 2 new test suites (`GraphicsPushConstantsTests` 5/5, `VisibilityCacheHashTests` 5/5) for hot-invariant functions. **Tier 5.8 verified:** InputAction bit-mask already correct (1ull << actionIndex + static_assert(kInputActionCount <= 64)). Build green, ctest **16/16 passed** (was 14/14, +2 new suites), FrustumCullBenchmark bit-identical verified. **Next:** remaining Tier 5 items (Tier 5.6 AppState PIMPL — large refactor, Tier 5.3 shader DDA template, Tier 5.5 vkWaitForFences timeout on remaining call sites, Tier 5.9 std::array → std::span) или переход к другим Tier'ам.

Updated: `2026-06-20` — **Tier 1.E InputReplay paths std::string → std::filesystem::path r0** (session `session-2026-06-20T-tier1e-path-migration-r0`, commit `20b2d9e`). `InputReplayCapture::snapshotPath` + `InputReplayState::replayPath` мигрированы на `std::filesystem::path` (OS-native path semantics, portable SDL_Log через `.string().c_str()`). Audit: `ModelRegistryEntry::id` уже StringID; `VoxelScenePresetToString/ParseVoxelScenePreset` уже `std::string_view`/`std::optional`; `AudioEngine::m_currentTrackName/Artist/Title` намеренно НЕ migrate (cold-path display strings, StringID overhead не оправдан). `RuntimeDiagnostics::LogRuntimeFailure` уже `std::string_view`. File format backwards-compatible (std::quoted on .string()). Build green, ctest 14/14 (0.76 sec, baseline 0.78 noise). **Next:** продолжение плана — Tier 5.x (god-object refactor: AppState PIMPL, UpdateApp mirror helpers).

Updated: `2026-06-20` — **Tier 0.E C-kernel → orthodox C++ kernel r0** (session `session-2026-06-20T-tier0e-c-kernel-rename-r0`, commit `08de29d` + rename в `bafecf9`). Per оператор: «C код и C++ код с C-стилем одинаковы по скорости, лучше переименовать frustum_cull.c в .cpp». Rename + orthodox C++ rewrite: `<cstdint>`, `<cstddef>`, `<cmath>`, `noexcept`, `const T&`, `bool`, `[[likely/unlikely]]`, `projectv::c_kernels` namespace, drop `extern "C"`. Godbolt-equivalent verification через `objdump -d` на `libprojectv_c_kernels.a`: 23 VEX-encoded AVX2 instructions (vmovps/vmulps/vaddps/vcmpps/vbroadcastss), `__attribute__((target("avx2")))` сохранён. Bit-identical benchmark: `[Tier3 frustum_cull] bit-identical across all implementations (cpp, c_scalar, c_avx2)`. Performance: BM_CppScalar 511ns / BM_CScalar 167ns / BM_CAvx2 257ns — within noise. Build green, ctest 14/14.

Updated: `2026-06-20` — **Tier 0.D hot-vector reserve verification + minor [[unlikely]] r0** (session `session-2026-06-20T-tier0d-hot-vector-reserve-r0`, commit `bafecf9`). Audit: ВСЕ 8 hot-path vectors уже pre-reserved в предыдущих коммитах. ChunkVisibilityCache уже `std::array<..., 1024>` (Tier 1.A `std::inplace_vector` не нужен). Hardening: `[[unlikely]]` на `chunk.rebuildQueued` early-out в `QueueChunkRebuildRequest`. CHANGELOG.md documents verification audit. Build green, ctest 14/14 (0.76 sec, baseline 0.78 noise).

Updated: `2026-06-20` — **Tier 0.C frustum-cull template unification r0** (session `session-2026-06-20T-tier0c-frustum-cull-template-r0`, commit `af69d06`). 2 cull-функции в `src/render/SceneResources.hpp:21-215` (`IsSceneChunkVisible` + `IsAabbVisibleAgainstCameraFrustum`) объединены в один template `FrustumCullVsCamera<GetOrigin, GetHalfExtent>(parameters, getOrigin, getHalfExtent)`. Public API сохранён — обе функции стали тонкими inline wrappers с capture-lambdas. Net -8 lines (DRY win: 200+ строк duplicated cull algorithm → 1 template + 2 thin wrappers). Branch hints: `[[unlikely]]` на near-plane fail (L73), maxDistance sphere fail (L80), side planes fail (L109), top/bottom fail (L112); `[[likely]]` на `if (maxDistance > 0.0f)` (L77). Lazy sqrt: `halfExtentRadius = sqrt(lengthSq(halfExtent))` вынесен из unconditional pre-compute в `if (maxDistance > 0.0f) [[likely]]` блок — экономия sqrt для chunks failing near-plane. `IsSceneChunkVisibleInShadowCascade` (L217-270) **не тронут** — другой алгоритм (8-corner transform + clip-space check), Tier 5 follow-up. **Build:** `cmake --build --clean-first` 1003+ targets green, 0 errors, 0 new warnings. **ctest:** 14/14 passed, 0.83 sec (baseline 0.78 noise). **FrustumCullBenchmark main():** `[Tier3 frustum_cull] bit-identical across all implementations (cpp, c_scalar, c_avx2)` — template-derived cpp_scalar matches C kernel bit-exactly. **Performance:** BM_CppScalar 534 ns, BM_CScalar 165 ns (3.2×), BM_CAvx2 240 ns (small fixture; AVX2 wins on larger batches). Stuck loop 0 compile-fail итераций. **Next:** Tier 0.D (pre-reserve / `inplace_vector` для hot vectors: `VoxelWorld.hpp:93` `pendingChunkRebuildIndices`, `SceneResources.hpp:ChunkVisibilityCache.opaqueCommands/shadowCommands/transparentCommands`) ждёт команды «продолжай Tier 0.D».

Updated: `2026-06-20` — **Tier 0.B hot-path Vec3/Vec4 migration r0** (session `session-2026-06-20T-tier0b-math-migration-r0`, commit `cf4b535`). 15 полей `std::array<float,N>` в `src/core/Types.hpp` мигрированы на `projectv::math::Vec3/Vec4` (alignas 16): 8×Vec3 (`CameraState::position`, `DebugStats::{walkFeetPosition,sunDirection,localPointLightPosition,Color}`, `ModelInstanceData::sourceAabbMin`, `ModelRegistryEntry::{aabbMin,aabbMax}`) + 7×Vec4 (`GraphicsPushConstants::{cameraPosition,cameraForward}`, `DebugOverlayPushConstants::{overlayData0,overlayData1,overlayColor}`, `DebugHudVertex::color`, `DebugOverlayBox::color`). Zero ABI-change для большинства struct'ов (same byte layout, только alignment tighten → `movaps` вместо `movups`). `DebugHudVertex` sizeof 24→32 bytes (color offset 8→16), GPU buffer auto-adjusted через `sizeof(DebugHudVertex)*262144 = 8 MB` в `SceneResources.cpp:838`. Downstream touch-up: 9 файлов — `Camera.cpp AddScaled(Float3*)`, `DebugHud.cpp/DebugOverlays.cpp` (14 constexpr + 2 сигнатуры → Vec4), `AppUpdate.cpp` walkFeetPosition assignment от `std::array<float,3>` через explicit `Vec3` ctor (boundary conversion, `PhysicsWalkDebugInfo::feetPosition` kept std::array), `PhysicsWorld.cpp` IsSolidAtPosition call site std::array literal (helper signature preserved), `VoxelInteraction.cpp` RaycastVoxelWorld origin array built from `Vec3`, `LookDevCaptureAutomation.cpp` аналогично, `tests/VoxelWorldTests.cpp` `ColorsMatch(Vec4,Vec4)` overload + 2 prevPosition literal conversions + 3 constexpr → Vec4. **Build:** `cmake --build --clean-first` 1003/1003 targets green, 0 errors, 0 new warnings. **ctest:** 14/14 passed, 0.78 sec (baseline 0.76 sec, noise level). Stuck loop 3 compile-fail итерации (DebugHud, DebugOverlays, tests ColorsMatch) — в пределах 3-4 лимита per `AGENTS.md §6.7`, не BLOCKED. Plan из 15 sub-tasks (Tier 0..5) одобрен оператором в Plan Mode, Tier 0.B — первая completed. **Next:** Tier 0.C (templated `FrustumCull<Lambda>` + `std::simd<float,8>` в `src/render/SceneResources.hpp:21-209`) ждёт команды «продолжай Tier 0.C».
...
- Active sub-plan: `TODO.md` Roadmap v1 (dependency-aware, 6 Stages, GPU-driven, flat list with detailed per-item approach, supersedes Tier 0..5 r0 per `2026-06-20` rewrite + dependency-mismatch reordering).
...
- **Deletions (3773 lines net, 273 unique blocks across 72 files)**: parser yielded 283 refactor-history blocks; 10 were duplicates (parse_blocks quirk on multi-blank-separated comment blocks) which dedup-by-(file,start,end) collapsed. Net removal = 3773 = 283 - 10 duplicates worth of repeated content.
- **Build (`linux-clang-debug`)**: 262/262 targets green, 0 errors, 0 new warnings. ctest 14/14 in 0.72 sec. Baseline preserved.
- **No scope overlap with `b44781e fix(physics): use JPH::CharacterContact after Jolt submodule bump` (parallel `session-2026-06-19T-jolt-api-drift-sweep-r0`)** — physics files untouched, only their Jolt API rename which I had no scope over.

**Phase B incident (informative for future agents):**
- Initial `apply_phase_b.py` v1 had NO dedup. The 10 duplicate blocks in inventory caused the script to apply the same deletion twice on the same file: the SECOND pass used stale line indices (the file was already shorter from the first pass) and accidentally deleted NON-comment content — e.g. `src/shaders/ray_march.comp` lost 44 lines of GLSL code (function signatures, layout declarations) along with the comment block at L3-L44. Build broke (`'gridDimsAndUnused' : unknown swizzle selection`, `'out' : not allowed in nested scope`).
- **Recovery:** I snapshotted all 72 affected files into `/tmp/phase_B_snapshot_HEAD/` before the destructive operation per `AGENTS.md §7.2.4` safety-net pattern. Restored from snapshot. Re-ran `apply_phase_b.py` v2 with `seen = {(file, start, end)}` dedup at the top of `apply_deletions()`. Deletion went 3773 lines, build green, ctest 14/14. Lesson: block-based parsers for large refactor passes need explicit dedup, and the §7.2.4 safety-net paid off.
- **Future mitigation:** `apply_phase_b.py` ships with the dedup block as a permanent fix (re-runnable idempotent).

**Files (Phase A + B, 1 new tool, 1 new data, 1 new CHANGELOG, 0 modified, 1 untracked inventory):**
- `tools/scratch/inventory_comments.py` (NEW, ~430 lines) — pure-read classifier, no suppressions per `AGENTS.md §7.2.7`. Walks `src/`, `tests/`, `src/shaders/` recursively (163 files, 103 with comments), groups consecutive `//` lines into blocks (allowing 1 blank-line separator), classifies via regex precedence `keep > refactor-history > test-narrative > design-rationale > intent`. **Untracked, throwaway.**
- `tools/scratch/apply_phase_b.py` (NEW, ~190 lines) — generate CHANGELOG.md + apply deletions with built-in dedup. **Untracked, throwaway.**
- `tools/scratch/comment-inventory.{json,csv}` (NEW, ~990 KB + ~120 KB) — full structured data, all 1073 blocks with `file:line_range`, `line_count`, `category`, `keywords_matched`, `preview`. **Untracked, throwaway.**
- `tools/scratch/SUMMARY*.md` (NEW) — stdout capture of inventory summary. **Untracked, throwaway.**
- `CHANGELOG.md` (NEW, 392 lines, 26.6 KB) — **COMMITTED** in `26c1a05`. This is the canonical reference for refactor-history going forward.
...
**Final result (`0fa26f4`, 50 files, +880/-830, amended from `aecf7f3` after close-routine added `agent/active-sessions.md` + `agent/status.md`):**
