...
// measures the per-frame cost of `BuildSunShadowProjection`
// and `BuildSunShadowCascadeSplits`, the two shadow
// projection hot paths called from the per-frame
// cascade-rebuild path. The output is a numeric `ns/op`
// (not pass/fail), used for perf regression tracking
// per `decisions.md §777` "Google Benchmark'и для всех
// hot path".
//
// **Complementary to ctest, not a replacement.**
// - ctest (`ProjectVSunShadowCascadeSplitsTests`):
//   verifies the algorithm's *correctness* invariants
//   (monotonic splits, forced `farPlane` at the last
//   slot, lambda clamp, NaN defensive defaults).
//   Boolean, fast, runs on every CI build.
// - Google Benchmark (this binary): verifies the
//   algorithm's *performance* over time. Numeric
//   (ns/op + stddev), 1+ second per measurement,
//   run manually or on nightly perf CI.
//
// The two together give a "did we break correctness
// AND did we break performance?" matrix.
//
// **Test fixture.** A 24x16x24 voxel world
// (8x8x8 chunks = 12 chunks, 9216 voxels) with a
// solid Y=0 floor. The Y=0 floor ensures
// `BuildSunShadowProjection`'s `TryGetActiveSceneBounds`
// finds non-trivial scene bounds (an all-Air world
// would short-circuit to the world min/max, which
// is the degenerate path we don't want to measure).
// The world is built once outside the `for (auto _ :
// state)` loop — the construction cost is amortised
// across all iterations.
//
// **Why no `BuildSunShadowCascadeProjections`
// benchmark.** That helper runs once per
// `kSunShadowCascadeCount = 4` cascades per frame.
// It's a superset of `BuildSunShadowProjection`
// (per-cascade + light VP construction), so the
// latter's benchmark is a lower bound on the former's
// cost. A future sub-task can add the cascade variant
// if its cost becomes observable in the Tracy plot.
