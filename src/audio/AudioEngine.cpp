#include "audio/AudioEngine.hpp" // pre-reset rationale: legacy/docs/archive/2026-06-24-pre-reset-snapshot/COMMENTS.md
...
void AudioEngine::RefreshPlaylistAsync()
{
	...
	m_scanThread = std::jthread([this](const std::stop_token &stopToken) {
		...
		const std::lock_guard lock(m_playlistMutex);
		if (stopToken.stop_requested()) {
			return;
		}
		scanPlaylist();
