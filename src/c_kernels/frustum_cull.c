// **Tier 3 (`2026-06-13`).** `frustum_cull.c` — scalar
// reference + AVX2 8-way kernel. The scalar version is
// the verification baseline: the AVX2 version must produce
// bit-identical output for the same inputs. The benchmark
// (`bench/FrustumCullBenchmark.cpp`) compares the AVX2
// kernel against the C++ helper in
// `src/render/SceneResources.hpp`.
//
// **Why C, not C++.** The hot path is six dot products per
// AABB. A C kernel with AVX2 intrinsics + `__attribute__
// ((target("avx2")))` per function gives:
// - one function per kernel — no vtable, no inlining budget
// - one translation unit — Godbolt + Clang 22 diagnostics
//   map 1:1 to the source
// - `extern "C"` boundary — caller-side (C++) passes POD
//   parameters, no name-mangling surprises
// - per-function AVX ABI — Clang 22 issue #194008 is for
//   whole-TU AVX2 + ASan; per-function `target` attribute
//   sidesteps it (the `IsAabbVisibleAgainstCameraFrustum`
//   C++ helper still gets `-mavx2` from the global compile
//   options, but the kernel is opt-in per call).
//
// **Layout.**
// - 6 plane normals (forward, left, right, bottom, top,
//   computed at function entry from the camera basis +
//   FOVs, all 3-component unit-length-ish vectors in
//   world space).
// - Per AABB: center (3), half_extent (3),
//   to_center = center - cameraPos (3).
// - Per plane: cull = (dot(toCenter, n) - planeOffset)
//   + (|n.x| * he.x + |n.y| * he.y + |n.z| * he.z) < 0.
//   The AABB is in the frustum iff NOT culled by any plane.
//
// **AVX2 vectorisation.**
// - 8 AABBs in parallel; one `__m256` per AABB-axis value
//   (8 x.x, 8 x.y, 8 x.z) gives 3 SIMD registers for the
//   whole batch of 8.
// - For each plane: broadcast the 3 plane-normal
//   components as scalar floats, multiply by the 8
//   AABB-axis values, sum the 3 products → 8 dots
//   (1 lane per AABB). abs-of-normal is precomputed
//   once per kernel call (not per plane), multiplied
//   by the 8 half-extent axes, summed → 8 projected
//   radii. compare `dots + radii < 0` → 8-bit mask,
//   OR'd into a per-batch "culled" bitmask.
// - 6 planes + maxDistance test = 7 masks. Final bit
//   = `~culled & 0xFF` → packed into the output mask
//   lane by lane.
//
// **Verification contract.** `projectv_cull_frustum_scalar`
// is the reference; it must match the C++ helper for all
// inputs. `projectv_cull_frustum_avx2` must match
// `projectv_cull_frustum_scalar` bit-by-bit. The benchmark
// asserts this on a small fixture before measuring
// throughput.
