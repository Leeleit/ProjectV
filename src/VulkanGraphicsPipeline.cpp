...
bool CreateDebugOverlayPipeline(
	...
	RenderState &render)
{
	std::vector<char> vertexShaderCode = ReadFile("debug_overlay.vert.spv");
	std::vector<char> fragmentShaderCode = ReadFile("debug_overlay.frag.spv");
	...
	if (vkCreatePipelineLayout(context.device, &pipelineLayoutInfo, nullptr, &render.debugOverlayPipelineLayout) != VK_SUCCESS) {
		vkDestroyShaderModule(context.device, vertexShaderModule, nullptr);
		vkDestroyShaderModule(context.device, fragmentShaderModule, nullptr);
		return false;
	}
	...
	const bool pipelineCreated =
		vkCreateGraphicsPipelines(context.device, VK_NULL_HANDLE, 1, &pipelineInfo, nullptr, &render.debugOverlayPipeline) == VK_SUCCESS;
	vkDestroyShaderModule(context.device, vertexShaderModule, nullptr);
	vkDestroyShaderModule(context.device, fragmentShaderModule, nullptr);
	if (!pipelineCreated) {
...
bool CreateDebugHudPipeline(
	...
	RenderState &render)
{
	std::vector<char> vertexShaderCode = ReadFile("debug_hud.vert.spv");
	std::vector<char> fragmentShaderCode = ReadFile("debug_hud.frag.spv");
	if (vertexShaderCode.empty() || fragmentShaderCode.empty()) {
		return false;
	}
	...
	VkPipelineLayoutCreateInfo pipelineLayoutInfo{};
	pipelineLayoutInfo.sType = VK_STRUCTURE_TYPE_PIPELINE_LAYOUT_CREATE_INFO;
	if (vkCreatePipelineLayout(context.device, &pipelineLayoutInfo, nullptr, &render.debugHudPipelineLayout) != VK_SUCCESS) {
		vkDestroyShaderModule(context.device, vertexShaderModule, nullptr);
		vkDestroyShaderModule(context.device, fragmentShaderModule, nullptr);
		return false;
	}
	...
	const bool pipelineCreated =
		vkCreateGraphicsPipelines(context.device, VK_NULL_HANDLE, 1, &pipelineInfo, nullptr, &render.debugHudPipeline) == VK_SUCCESS;
	vkDestroyShaderModule(context.device, vertexShaderModule, nullptr);
	vkDestroyShaderModule(context.device, fragmentShaderModule, nullptr);
	if (!pipelineCreated) {
