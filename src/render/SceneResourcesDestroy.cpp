	...
	RenderState *render)
{
	...
	for (auto &[packedFaceMappedData,
				packedFaceBuffer,
				packedFaceAllocation,
				chunkDescriptorMappedData,
				chunkDescriptorBuffer,
				chunkDescriptorAllocation,
				chunkVoxelPayloadMappedData,
				chunkVoxelPayloadBuffer,
				chunkVoxelPayloadAllocation,
				opaqueIndirectMappedData,
				opaqueIndirectBuffer,
				opaqueIndirectAllocation,
				transparentIndirectMappedData,
				transparentIndirectBuffer,
				transparentIndirectAllocation,
				dirtyChunkIndexMappedData,
				dirtyChunkIndexBuffer,
				dirtyChunkIndexAllocation,
				chunkCullingMappedData,
				chunkCullingBuffer,
				chunkCullingAllocation,
				sceneLightingMappedData,
				sceneLightingBuffer,
				sceneLightingAllocation,
				chunkAabbMappedData,
				chunkAabbBuffer,
				chunkAabbAllocation,
				visibilityMaskMappedData,
				visibilityMaskBuffer,
				visibilityMaskAllocation,
				hzbVisibleCountMappedData,
				hzbVisibleCountBuffer,
				hzbVisibleCountAllocation,
				hzbPerChunkMipMappedData,
				hzbPerChunkMipBuffer,
				hzbPerChunkMipAllocation,
				hzbPerChunkMipCapacityBytes,
				prevVisibilityMaskMappedData,
				prevVisibilityMaskBuffer,
				prevVisibilityMaskAllocation,
				opaqueHzbDrawIndirectMappedData,
				opaqueHzbDrawIndirectBuffer,
				opaqueHzbDrawIndirectAllocation,
				transparentHzbDrawIndirectMappedData,
				transparentHzbDrawIndirectBuffer,
				transparentHzbDrawIndirectAllocation,
				hizApplyDescriptorSet,
				hizApplyDescriptorSetInitialized,
				lodDownsampledVoxelPayloadMappedData,
				lodDownsampledVoxelPayloadBuffer,
				lodDownsampledVoxelPayloadAllocation,
				lodDownsampledVoxelPayloadCapacityBytes,
				chunkLodLevelsMappedData,
				chunkLodLevelsBuffer,
				chunkLodLevelsAllocation,
				chunkLodLevelsCapacity,
				visibleChunkIdMappedData,
				visibleChunkIdBuffer,
				visibleChunkIdAllocation,
				visibilityCounterMappedData,
				visibilityCounterBuffer,
				visibilityCounterAllocation,
				nanovdbUpperMappedData,
				nanovdbUpperBuffer,
				nanovdbUpperAllocation,
				nanovdbUpperCapacityBytes,
				nanovdbLowerMappedData,
				nanovdbLowerBuffer,
				nanovdbLowerAllocation,
				nanovdbLowerCapacityBytes,
				nanovdbLeafMappedData,
				nanovdbLeafBuffer,
				nanovdbLeafAllocation,
				nanovdbLeafCapacityBytes,
				nanovdbMaterialMappedData,
				nanovdbMaterialBuffer,
				nanovdbMaterialAllocation,
				nanovdbMaterialCapacityBytes,
				worldGenVoxelMappedData,
				worldGenVoxelBuffer,
				worldGenVoxelAllocation,
				worldGenVoxelCapacityBytes,
				meshClusters,
				worldGenDescriptorSet,
				graphicsDescriptorSet,
				meshShaderDescriptorSet,
				voxelMeshingDescriptorSet,
				hizCullingDescriptorSet,
				vctVoxelizeDescriptorSet,
				uploadedSceneVersion,
				uploadedVoxelPayloadVersion,
				meshedSceneVersion,
				uploadedNanoVdbVersion,
				chunkDescriptorCount,
				dirtyChunkCount,
				opaqueFaceCount,
				transparentFaceCount] : render->sceneFrameResources) {
		...
		if (prevVisibilityMaskBuffer && prevVisibilityMaskAllocation) {
			profiling::RecordFree(prevVisibilityMaskAllocation, "ScenePrevVisibilityMaskBufferAllocation");
			vmaDestroyBuffer(context->allocator, prevVisibilityMaskBuffer, prevVisibilityMaskAllocation);
		}
		if (opaqueHzbDrawIndirectBuffer && opaqueHzbDrawIndirectAllocation) {
			profiling::RecordFree(opaqueHzbDrawIndirectAllocation, "SceneOpaqueHzbDrawIndirectBufferAllocation");
			vmaDestroyBuffer(context->allocator, opaqueHzbDrawIndirectBuffer, opaqueHzbDrawIndirectAllocation);
		}
		if (transparentHzbDrawIndirectBuffer && transparentHzbDrawIndirectAllocation) {
			profiling::RecordFree(transparentHzbDrawIndirectAllocation, "SceneTransparentHzbDrawIndirectBufferAllocation");
			vmaDestroyBuffer(context->allocator, transparentHzbDrawIndirectBuffer, transparentHzbDrawIndirectAllocation);
		}
		...
		if (meshClusters.faceClusterBuffer && meshClusters.faceClusterAllocation) {
			profiling::RecordFree(meshClusters.faceClusterAllocation, "SceneFaceClusterBufferAllocation");
			vmaDestroyBuffer(context->allocator, meshClusters.faceClusterBuffer, meshClusters.faceClusterAllocation);
		}
		if (meshClusters.faceClusterCountBuffer && meshClusters.faceClusterCountAllocation) {
			profiling::RecordFree(meshClusters.faceClusterCountAllocation, "SceneFaceClusterCountBufferAllocation");
			vmaDestroyBuffer(context->allocator, meshClusters.faceClusterCountBuffer, meshClusters.faceClusterCountAllocation);
		}
		if (meshClusters.meshDrawIndirectBuffer && meshClusters.meshDrawIndirectAllocation) {
			profiling::RecordFree(meshClusters.meshDrawIndirectAllocation, "SceneMeshDrawIndirectBufferAllocation");
			vmaDestroyBuffer(context->allocator, meshClusters.meshDrawIndirectBuffer, meshClusters.meshDrawIndirectAllocation);
		}
		meshClusters = {};
		...
		prevVisibilityMaskMappedData = nullptr;
		prevVisibilityMaskBuffer = VK_NULL_HANDLE;
		prevVisibilityMaskAllocation = nullptr;
		opaqueHzbDrawIndirectMappedData = nullptr;
		opaqueHzbDrawIndirectBuffer = VK_NULL_HANDLE;
		opaqueHzbDrawIndirectAllocation = nullptr;
		transparentHzbDrawIndirectMappedData = nullptr;
		transparentHzbDrawIndirectBuffer = VK_NULL_HANDLE;
		transparentHzbDrawIndirectAllocation = nullptr;
		hizApplyDescriptorSet = VK_NULL_HANDLE;
		hizApplyDescriptorSetInitialized = false;
