...
bool CreateCloudscapeResources(VulkanContextState *context, RenderState *render)
{
	...
	{
		// Transition linear-tiled noise image to SHADER_READ_ONLY_OPTIMAL before
		// it is bound as a combined image sampler in the cloudscape fragment shader.
		...
		if (vkAllocateCommandBuffers(context->device, &allocInfo, &cmd) == VK_SUCCESS) {
			...
			if (vkBeginCommandBuffer(cmd, &beginInfo) == VK_SUCCESS) {
				...
				if (vkQueueSubmit2(context->queue, 1u, &submitInfo, VK_NULL_HANDLE) == VK_SUCCESS) {
	...
	if (vkCreateGraphicsPipelines(context->device, context->pipelineCache, 1u, &pipelineInfo, nullptr, &render->cloudscapePipeline) != VK_SUCCESS) {
		DestroyCloudscapeResources(context, render);
		return false;
	}
	...
	for (uint32_t i = 0; i < MAX_FRAMES_IN_FLIGHT; ++i) {
		const VkDescriptorImageInfo noiseInfo{
			render->cloudscapeLinearSampler,
			render->cloudscapeNoiseView,
			VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL,
		};
		const VkDescriptorImageInfo sceneColorInfo{
			render->cloudscapeLinearSampler,
		...
		writes[0].dstSet = render->cloudscapeDescriptorSets[i];
		...
		writes[1].dstSet = render->cloudscapeDescriptorSets[i];
		...
		writes[2].dstSet = render->cloudscapeDescriptorSets[i];
