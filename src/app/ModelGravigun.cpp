...
bool RayAabbIntersect(const Ray &ray, const glm::vec3 &aabbMin, const glm::vec3 &aabbMax, float &outTNear, float &outTFar)
{
	...
	bool intersect = true;
	for (int axis = 0; axis < 3 && intersect; ++axis) {
...
bool GravigunSnapEnabled()
{
	static const bool kSnapEnabled = [] {
		const char *value = core::GetEnvVar("PROJECTV_GRAVIGUN_SNAP");
		if (value == nullptr) {
			return false;
		}
	...
	InputState &input)
{
	...
	const bool nothingPicked = state.pickedInstanceIndex < 0;
	if (fPressed && nothingPicked) {
	...
	const bool somethingPicked = state.pickedInstanceIndex >= 0;
	if (somethingPicked && !fDown) {
