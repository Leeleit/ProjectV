...
int main()
{

	{
		...
		Expect(
			std::abs(pc.cameraPosition[0] - 0.0f) <= kEpsilon,
			"identity: cameraPosition.x == 0");
		Expect(
			std::abs(pc.cameraPosition[1] - 0.0f) <= kEpsilon,
			"identity: cameraPosition.y == 0");
		Expect(
			std::abs(pc.cameraPosition[2] - 0.0f) <= kEpsilon,
			"identity: cameraPosition.z == 0");
		Expect(
			std::abs(pc.cameraPosition[3] - 0.1f) <= kEpsilon,
			"identity: cameraPosition.w == nearPlane");
		Expect(
			std::abs(pc.cameraForward[3] - 128.0f) <= kEpsilon,
